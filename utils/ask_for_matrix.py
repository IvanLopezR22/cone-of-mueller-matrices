...
def ask_for_matrix(matrix_name):
    # main_matrix=Matrix([[1,2,3,0],[0,-1,5,7],[0,-1,0,9],[1,2,3,4]])
    print("Introduzca los valores de la matriz " + matrix_name + ":")
    matriz_A = []
    for raw_position in range(4):
        ...
        for element in range(4):
            while True:
                try:
                    raw.append(Rational(
                        str(float((input(f"Introduce el elemento {raw_position+1}, {element+1}: "))))))
                except ValueError:
                    print("A non-numeric character was entered, please enter a number.")
                    continue
                else:
                    break
...
def ask_for_matrix_interface(matrix_name):
    final_matrix = []
    l1 = psg.Text('Ingrese los valores de su matriz ' + matrix_name + ':', key='-OUT-', font=('Arial Bold',
    ...
    c44 = psg.Input('', enable_events=True, key='-INPUT-44',
                    font=('Arial Bold', 12), size=(12, 1), justification='left')
    b1 = psg.Button('Aceptar', key='-OK-', font=('Arial Bold', 12))
    b2 = psg.Button('Salir', font=('Arial Bold', 12))
    ...
    window = psg.Window('Ingrese la matriz', layout, size=(690,200))
    while True:
        event, values = window.read()
        if event.startswith('-INPUT-'):
            if len(values[event]) > 0 and values[event][-1] not in ('0123456789.-'):
                psg.popup("Inserta solo numeros/decimales")
                window[event].update(values[event][:-1])
        if event == ('-OK-'):
            final_matrix.clear()
            # Validación para elementos vacios
            for item in values:
                if values[item] == '':
                    psg.popup("No puedes dejar elementos vacios")
                    print(item + " vacio")
                    break
                else:
                    try:
                        ...
                        psg.popup("Entrada incorrecta, revisa los datos")
