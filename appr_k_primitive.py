...
from utils.k_primitive import know_k_primitive
...
def appr_k_primitive(w, main_matrix):
    ...
    ask_primitive = know_k_primitive(main_matrix)

    # This is the case in which the matrix M is K-primitive, then we leave M unchanged.
    if ask_primitive:
        m_primitive = main_matrix
        print(f"The introduced matrix is K-primitive. Then the approximation is the same M:"
    ...
    # Then we have an approximation M(prim) that we call the approximation of M by a K-primitive matrix.
        ...
        print(f"The K-primitive algorithm applied to {w}(prim) gives as a result: {know_k_primitive(m_primitive)}")
